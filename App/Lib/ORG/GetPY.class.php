...
class GetPY{
    ...
    public function getFirstPY($chinese){		
		$result = '';	
		if(preg_match('/^([\x81-\xfe][\x40-\xfe])+/' , $chinese)){		
		     $chinese = iconv("UTF-8","GB2312//IGNORE",$chinese);	
			 $chinese = mb_substr($chinese,0,1,'GB2312');	
			...
			$chinese = substr($chinese,0,1);
			$result .= $chinese;			
		}		
		$result = in_array(strtolower($result),$this->letter) ? $result : '#';
